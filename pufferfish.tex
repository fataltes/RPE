...
trade off search speed for index size. \fatemeh{The sparse index tuned to take maximum space is in the same level as dense index regarding space and \kmer query time. In both variants of the \pufferfish we utilize the succinct data structures and their specifically adopted operations like \rank and \select to boost the \kmer search query performance.}
...
reference sequences as ``paths''.  \fatemeh{Each path is spelled out with an ordered set of contig IDs and the orientation they are mapped by, so that each contig has an overlap of k-1 with its following contig in the path (either in forward or reverse-complement). }The GFA is an evolving standard that is meant
...
\item The contig sequence array (\cseq) consists of the (2-bit encoded) sequence of all
  ...
  contig boundaries in the packed array \fatemeh{as the contig sequences in the array follow each other without any delimiters or gaps.}). \fatemeh{Therefore, using the next data structure we will  set the boundaries of contigs in the array in a way that makes retrieving the borders, knowing position of a contig in the contig array and length of the contig fast in the query process} We denote by $L_s$ the total length (in
...
\item The boundary vector (\bv) is a sparse bit-vector with a length of $L_s$
  ...
  a zero everywhere else. \fatemeh{We can retrieve rank of each contig in \cseq using the \rank operation on \bv. $\rank(\bv[i])$ returns number of 1s in \bv before the current index, $i$, or in other words the rank of the current contig. This can be used to get reference information for the current contig from \ctab, the data structure explained in item \ref{items:dense5}.}
...
\item The position vector (\emph{pos}) stores, for each valid \kmer $x$, the
  ...
  \emph{pos}$\left[h\left(x\right)\right]$ contains the \fatemeh{starting} position of $x$ in \cseq 
...
\item The conitg table \ctab stores, for each contig appearing in \cseq, the reference
  ...
  via a simple rank operation on \bv. \fatemeh{Having the rank for each contig, we just directly go to the corresponding line and read the reference list in that line.}
...
\todo{shouldn't we switch the position of select and pi? or we should at least explain that select will return the last index of the previous contig??},
...
the right. \fatemeh{In addition we need a bit vector with the same size as \cseq
...
\caption{\fatemeh{An illustration of searching for a particular \kmer in the \emph{sparse}
  ...
  same as the one we searched.}\todo{The input kmer in the figure is not correct. It should be canonicalized}}
...
\fatemeh{At the end we will explain how the sparse indexing structure is preserving 
considerable space per \kmer that is not sampled although it has additional bit vectors
compared to the dense index.
...
$\frac{s}{2}$ bases for a kmer with each base represented in 2 bits, it'll be at the end 
$\frac{s}{2}\times 2 = s$ bits per \kmer for the extension vector. Therefore, if we put all the new 
vectors of extension, canonical, and direction together it'll be $s+1+1$ bits 
per each non-sampled \kmer compared to $\lg(L_s)$ bits that we had to keep in 
\emph{pos} vector. Therefore, as long as $[s+1+1+1 < \lg(L_s)]$ we save space by sparse indexing.
In a typical data set such as human genome with $\lg(L_s) ~= 20$ bits, by choosing
$s=9$ which means we'll sample each 9th \kmer, we will save $9$ bits per each non-sampled \kmer
or in other words for $90\%$ of the \kmers that in human genome data set with about 
1,000,000,000 \kmers we can improve memory by $\sim2$ gigabytes of space
even considering that $1$ bit per all the \kmers
that specifies whether a \kmer is sampled or not.}\todo{put actual numbers}

\section{\fatemeh{Results}}\label{sec:results}
...
We note that on large sequences (e.g., variants of the human genome containing patches and alternative haplotypes)
...
\todo{paragraphs above are both copies from the pufferfish poster abstract. Saying that in case they need to be modified!}

\fatemeh{Having built an index for a reference genome or transcriptome using \pufferfish, the immediate future work would be implementing the applications of such an index. These applications fall into the categories that need mapping or alignment as their initial step. Therefore, we would like to adopt an existing aligner/mapper such as RapMap~\cite{srivastava2016rapmap} or Selective Aligner~\cite{sarkar2017towards} and modify it to use \pufferfish as its indexing methodology. Later we can use the result of this aligner for massive RNA-sequence quantification or metagenomic abundance estimation tasks. We expect the efficient memory taking of \pufferfish indexing will be beneficial in working with bigger collections of metagenomic or RNA transcriptomes. Moreover, by indexing genome and transcriptome together using \pufferfish we can discover novel exon splicing. Genome comparison and structural variant detection in a group of genomes can also be one of the immediate results of the \pufferfish as construction and query time are both reasonably fast and also memory-efficient.}
